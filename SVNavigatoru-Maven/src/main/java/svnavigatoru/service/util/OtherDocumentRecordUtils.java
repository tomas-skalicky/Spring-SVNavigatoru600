...
public class OtherDocumentRecordUtils {
	...
	/**
	 * Gets an array of indicators which say which
	 * {@link OtherDocumentRecordType}s are checked, and which types are not.
	 * The array is filled in according to the given array of checked
	...
	/**
	 * <p>
	 * Gets the default array of indicators which say which
	 * {@link OtherDocumentRecordType}s are checked (selected), and which types
	 * are not.
	 * </p>
	 * <b>Precondition:</b> Ordinal values of all
	 * {@link OtherDocumentRecordType}s are exactly in the range of
	...
	/**
	 * Converts the given array of check <code>indicators</code> to the
	 * {@link Set} of {@link OtherDocumentRecordTypeRelation}s associated with
	 * an {@link OtherDocumentRecord} with the given <code>recordId</code>.
	 * 
	 * @return Set of those record types which have been checked, i.e. their
	 *         indicators equal <code>true</code>.
	 */
	public static Set<OtherDocumentRecordTypeRelation> convertIndicatorsToRelations(boolean[] indicators, int recordId) {
