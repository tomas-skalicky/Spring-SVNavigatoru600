...
public class JspCodeGenerator {
	 ...
	 *            The reference of the navigation item.
	 * @param rootAddress
	 *            The part of the URL address which is common for all sub-items
	 *            of the root item.
	 * @param title
	 *            The title of the navigation item.
	 * @param request
	...
	private static String getMainNavigationItem(String reference, String rootAddress, String title,
		...
		return JspCodeGenerator.getMainNavigationItem(reference, rootAddress, title, request, new String[][] {});
	}
	 ...
	 * @param request
	 * @param subItems
	 *            The array of information about sub-items of the root
	 *            navigation item. Each sub-item is specified by an array of
	 *            {@link String}s where on the first position there is a
	 *            reference of the item and on the second position there is its
	 *            title.
	 */
	public static String getMainNavigationItem(String reference, String title, HttpServletRequest request,
			String[][] subItems) {
	...
	public static String getMainNavigationItem(String reference, String rootAddress, String title,
		...
		return String.format("<li>%s</li>",
				JspCodeGenerator.getMainNavigationItem(reference, rootAddress, title, request, true, subItems));
	}

	/**
	 * Gets the JSP code of an item of the main navigation. The code does not
	 * contain <code>&lt;li></code> tag.
	 * 
	 * @param reference
	 *            The reference of the navigation item.
	 * @param rootAddress
	 ...
	 * @param request
	 * @param isRoot
	 *            Indicates whether the navigation item which is to be returned
	 *            is one of the root items.
	 * @param subItems
	 *            The array of information about sub-items of the navigation
	 *            item. Each sub-item is specified by an array of {@link String}
	 *            s where on the first position there is a reference of the item
	...
	private static String getMainNavigationItem(String reference, String rootAddress, String title,
		...
		jspCode.append(String.format("<a href=\"%s/%s/\" class=\"fadeThis %s\">", homeUrl, reference, cssClasses));
		...
		if (subItems.length > 0) {
			...
			for (String[] subItem : subItems) {
				jspCode.append(String.format("<li>%s</li>", JspCodeGenerator.getMainNavigationItem(subItem[0], "",
						subItem[1], request, false, new String[][] {})));
			}
			jspCode.append("</ul>");
		}
