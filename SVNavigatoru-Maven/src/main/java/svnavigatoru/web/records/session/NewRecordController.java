...
public abstract class NewRecordController extends NewEditRecordController {
	...
	public String initForm(SessionRecordType recordType, HttpServletRequest request, ModelMap model) {
		...
		command.setNewType(Localization.findLocaleMessage(this.messageSource, request, recordType.getLocalizationCode()));
	...
	/**
	 * If values in the form are OK, the new record is stored to the repository.
	...
	public String processSubmittedForm(NewSessionRecord command, BindingResult result, SessionStatus status,
		...
		newRecord.setType(SessionRecordType.valueOfAccordingLocalization(command.getNewType(), this.messageSource,
				request));
		...
		try {
			...
			model.addAttribute(Configuration.REDIRECTION_ATTRIBUTE, String.format("%svytvoreno/", this.BASE_URL));
