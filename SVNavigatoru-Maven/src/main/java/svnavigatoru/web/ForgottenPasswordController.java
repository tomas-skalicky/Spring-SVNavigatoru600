...
import svnavigatoru.service.users.SendNewPassword;
import svnavigatoru.service.users.SendNewPasswordValidator;
...
public class ForgottenPasswordController extends MetaController {
	...
	/**
	 * If values in the form are OK, generates a new password, stores it to a
	 * repository and sends it to the provided email address. If something is
	 * wrong, returns back to the form.
	 * 
	 ...
	 * <code>emailAddress</code>. The function is invoked when user's password
	...
	private void sendEmailOnPasswordReset(String emailAddress, String lastName, String newPassword,
		...
		String subject = Localization.findLocaleMessage(this.messageSource, request, "email.subject.password-reset");
		Object[] messageParams = new Object[] { lastName, Configuration.DOMAIN, newPassword };
		String messageText = Localization.findLocaleMessage(this.messageSource, request, "email.text.password-reset",
				messageParams);
