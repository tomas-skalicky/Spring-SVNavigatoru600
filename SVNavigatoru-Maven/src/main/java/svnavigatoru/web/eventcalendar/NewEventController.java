...
import svnavigatoru.service.eventcalendar.NewEvent;
import svnavigatoru.service.eventcalendar.NewEventValidator;
...
public class NewEventController extends NewEditEventController {
	...
	/**
	 * If values in the form are OK, the new calendar event is stored to the
	...
	public String processSubmittedForm(@ModelAttribute(NewEventController.COMMAND) NewEvent command,
		...
		newEvent.setPriority(PriorityType.valueOfAccordingLocalization(command.getNewPriority(), this.messageSource,
				request));
