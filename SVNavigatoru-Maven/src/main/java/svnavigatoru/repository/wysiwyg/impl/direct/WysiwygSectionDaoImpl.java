...
public class WysiwygSectionDaoImpl extends SimpleJdbcDaoSupport implements WysiwygSectionDao {
	...
	public void update(WysiwygSection section) {
		...
		String query = String.format("UPDATE %s SET %s = ?, %s = ? WHERE %s = ?", WysiwygSectionDaoImpl.TABLE_NAME,
				WysiwygSectionRowMapper.getColumn("lastSaveTime"), WysiwygSectionRowMapper.getColumn("sourceCode"),
				WysiwygSectionRowMapper.getColumn("name"));
