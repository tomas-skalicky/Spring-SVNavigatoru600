...
public class SessionRecordDaoImpl extends HibernateDaoSupport implements SessionRecordDao {
	...
	public SessionRecord findByFileName(String fileName) {
		...
		List<SessionRecord> records = (List<SessionRecord>) this.getHibernateTemplate().findByCriteria(criteria);
		if (records.size() > 1) {
	...
	public List<SessionRecord> findOrdered(OrderType order) {
		String query = String.format("FROM SessionRecord r ORDER BY r.sessionDate %s", order.getDatabaseCode());
