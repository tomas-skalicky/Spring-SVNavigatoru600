...
public class OtherDocumentRecordDaoImpl extends SimpleJdbcDaoSupport implements OtherDocumentRecordDao {
	...
	/**
	 * The SELECT command for the return of a single document together with its
	 * BLOB file.
	 * 
	 * Join is necessary in all SELECT queries since {@link OtherDocumentRecord}
	 * inherits from {@link DocumentRecord}.
	 */
	...
	/**
	 * The SELECT command for the return of a single or multiple documents
	 * without their BLOB files.
	 * 
	 * Join is necessary in all SELECT queries since {@link OtherDocumentRecord}
	 * inherits from {@link DocumentRecord}.
	 */
	private static final String SELECT_FROM_CLAUSE_WITHOUT_FILE = String.format(
			"SELECT r.*, d.%s FROM %s r INNER JOIN %s d ON d.%s = r.%s", DocumentRecordRowMapper.getColumn("fileName"),
			OtherDocumentRecordDaoImpl.TABLE_NAME, DocumentRecordDaoImpl.TABLE_NAME,
			DocumentRecordRowMapper.getColumn("id"), OtherDocumentRecordRowMapper.getColumn("id"));
	...
	/**
	 * Populates the <code>types</code> property of the given
	 * <code>record</code>.
	 */
	...
	/**
	 * Populates the <code>types</code> property of all the given
	 * <code>records</code>.
	 */
	...
	public OtherDocumentRecord findById(int recordId, boolean loadFile) {
		...
		String query = String.format("%s WHERE r.%s = ?", selectClause, OtherDocumentRecordRowMapper.getColumn("id"));
	...
	public OtherDocumentRecord findByFileName(String fileName) {
		String query = String.format("%s WHERE r.%s = ?", OtherDocumentRecordDaoImpl.SELECT_FROM_CLAUSE_WITH_FILE,
		...
		if (record == null) {
			throw new DataRetrievalFailureException(String.format("No record with the filename '%s' exists.", fileName));
		}
	...
	public void update(OtherDocumentRecord record) {
		...
		String query = String.format("UPDATE %s SET %s = ?, %s = ?, %s = ?, %s = ? WHERE %s = ?",
				...
				OtherDocumentRecordRowMapper.getColumn("lastSaveTime"), OtherDocumentRecordRowMapper.getColumn("id"));
		this.getSimpleJdbcTemplate().update(query, record.getName(), record.getDescription(), record.getCreationTime(),
				record.getLastSaveTime(), record.getId());
	...
	public int save(OtherDocumentRecord record) {
		...
		SimpleJdbcInsert insert = new SimpleJdbcInsert(this.getDataSource()).withTableName(
				OtherDocumentRecordDaoImpl.TABLE_NAME).usingColumns(OtherDocumentRecordRowMapper.getColumn("id"),
				OtherDocumentRecordRowMapper.getColumn("name"), OtherDocumentRecordRowMapper.getColumn("description"),
				OtherDocumentRecordRowMapper.getColumn("creationTime"),
		...
		if (types != null) {
			...
			// svnavigatoru.web.records.otherdocuments.NewDocumentController.
