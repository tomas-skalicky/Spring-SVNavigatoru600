...
/**
 * Provides convenient methods to work with {@link com.svnavigatoru600.domain.forum.ForumContribution Contribution}
 * objects.
 * <p>
...
public class ContributionService implements SubjectOfNotificationService {
    ...
    @PreAuthorize("hasPermission(#contributionId, '" + ForumContribution.CLASS_FULL_NAME + "', 'edit')")
    public void canEdit(final int contributionId) {
    }

    @PreAuthorize("hasPermission(#contributionId, '" + ForumContribution.CLASS_FULL_NAME + "', 'delete')")
     ...
     * <p>
     * NOT IMPLEMENTED YET: It returns only the first <code>maxResultSize</code> {@link ForumContribution
     * Contributions}.
     */
    public List<ForumContribution> findLimitedNumberOrdered(final int maxResultSize) {
        return contributionDao.findAllOrdered(ContributionFieldEnum.LAST_SAVE_TIME, OrderTypeEnum.DESCENDING,
                maxResultSize);
    }
    ...
    /**
     * Updates the given {@link ForumContribution} in the repository. The old version of this contribution should be
    ...
    /**
     * Updates properties of the given <code>contributionToUpdate</code> and persists this {@link ForumContribution}
     * into the repository. The old version of this contribution should be already stored there.
     *
     * @param contributionToUpdate
     *            Persisted {@link ForumContribution}
     * @param newContribution
     *            {@link ForumContribution} which contains new values of properties of
     ...
     */
    public void updateAndNotifyUsers(final ForumContribution contributionToUpdate,
            final ForumContribution newContribution, final boolean sendNotification, final HttpServletRequest request,
    ...
    /**
     * Updates corresponding {@link java.util.Date Date} fields of the given new {@link ForumContribution} (if there are
     * any appropriate) and stores the contribution to the repository.
     * <p>
     ...
     *            If <code>true</code>, the notification is sent; otherwise not.
     */
    public void saveAndNotifyUsers(final ForumContribution newContribution, final boolean sendNotification,
    ...
    @Override
    public void notifyUsersOfCreation(final Object newContribution, final HttpServletRequest request,
            final MessageSource messageSource) {
    ...
    /**
     * Gets a {@link Map} which for each input {@link ForumContribution} contains a corresponding localized delete
     * question which is asked before deletion of that contribution.
     */
    public static Map<ForumContribution, String> getLocalizedDeleteQuestions(
            final List<ForumContribution> contributions, final HttpServletRequest request,
            final MessageSource messageSource) {
