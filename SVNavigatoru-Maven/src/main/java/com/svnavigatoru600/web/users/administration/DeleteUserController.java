...
public class DeleteUserController extends UserController {
	...
	public String delete(@PathVariable String username, HttpServletRequest request, ModelMap model) {
		try {
			...
			model.addAttribute(Configuration.REDIRECTION_ATTRIBUTE, DeleteUserController.BASE_URL + "smazano/");
	...
	/**
	 * Sends an email to the given <code>user</code> that his account has been
	...
	private void sendEmailOnUserDeletion(User user, HttpServletRequest request) {
		...
		String subject = Localization.findLocaleMessage(this.messageSource, request, "email.subject.user-deleted");
		Object[] messageParams = new Object[] { user.getLastName(), user.getUsername(), Configuration.DOMAIN };
		String messageText = Localization.findLocaleMessage(this.messageSource, request, "email.text.user-deleted",
				messageParams);
