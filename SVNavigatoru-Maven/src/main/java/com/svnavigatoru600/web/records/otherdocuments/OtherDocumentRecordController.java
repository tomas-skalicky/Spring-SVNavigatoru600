...
public abstract class OtherDocumentRecordController extends DocumentRecordController {
	...
	/**
	 * If all {@link OtherDocumentRecord}s are considered (i.e. processed) by
	 * this controller, <code>allRecordTypes</code> equals <code>true</code>.
	...
	/**
	 * Gets a {@link Map} which for each {@link OtherDocumentRecordType} (more
	 * precisely its ordinal) contains an appropriate ID of its checkbox.
	 */
	...
	/**
	 * Gets a {@link Map} which for each {@link OtherDocumentRecordType} (more
	 * precisely its ordinal) contains an appropriate localized title of its
	 * checkbox.
	 */
	protected Map<Integer, String> getLocalizedTypeCheckboxTitles(HttpServletRequest request) {
		...
		for (OtherDocumentRecordType type : OtherDocumentRecordType.values()) {
			...
			checkboxTitles.put(type.ordinal(), Localization.findLocaleMessage(this.messageSource, request, titleCode));
		}
