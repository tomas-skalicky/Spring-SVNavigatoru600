...
public class DocumentRecordDaoImpl extends SimpleJdbcDaoSupport {
	...
	public void update(DocumentRecord record, DataSource dataSource) {
		...
		String query = String.format("UPDATE %s SET %s = ?, %s = ? WHERE %s = ?", DocumentRecordDaoImpl.TABLE_NAME,
	...
	public int save(DocumentRecord record, DataSource dataSource) {
		...
		SimpleJdbcInsert insert = new SimpleJdbcInsert(dataSource).withTableName(DocumentRecordDaoImpl.TABLE_NAME)
				...
				.usingColumns(DocumentRecordRowMapper.getColumn("fileName"), DocumentRecordRowMapper.getColumn("file"));

		// For more info, see repository.news.impl.direct.NewsDaoImpl.java
		Map<String, Object> keys = insert.executeAndReturnKeyHolder(this.getNamedParameters(record)).getKeys();
