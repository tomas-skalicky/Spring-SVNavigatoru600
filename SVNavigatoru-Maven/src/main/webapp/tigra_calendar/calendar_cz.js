...
var DEFAULTTEMPLATE = {
	...
	"weekStart": 1, // the first day of week: 0-Su or 1-Mo.
...
function CalendarParseDateStr(dateStr) {
	var dateRegExp = new RegExp("^\\s*(\\d{1,2})\\.(\\d{1,2})\\.(\\d{2,4})\\s*$");
	if (dateRegExp.test(dateStr) === false) {
		return alert("Chybný datum: '" + dateStr + "'.\nPřípustný formát je dd.mm.yyyy.");
	}
	
	var yearNum = new Number(RegExp.$3);
	var monthNum = new Number(RegExp.$2);
	var dayNum = new Number(RegExp.$1);
	...
	if (monthNum < 1 || monthNum > 12) {
		return alert("Chybný měsíc: '" + monthNum + "'.\nDovolený rozsah je 1 - 12.");
	}
	if (dayNum < 1 || dayNum > GetDayCount(yearNum, monthNum)) {
		return alert("Chybný den v měsíci: '" + dayNum + "'.\nDovolený rozsah pro vybraný měsíc je 1 - " + GetDayCount(yearNum, monthNum) + ".");
	}
...
function Calendar(config, template) {
	...
	if (!this.iconE) {
		document.write("<img src='" + config.imgPath + "calendar.png' id='" + this.iconEId + "' onclick=\"calendarsAssoc[\'" + this.id + "\'].Toggle()\" class='calendarIcon' title='Otevřít kalendář' alt='Otevřít kalendář' />");
		this.iconE = GetElement(this.iconEId);
	}
...
function CalendarShow() {
	...
	this.iconE.title = "Zavřít kalendář";
	this.visible = true;
}
...
function CalendarHide(timestamp) {
	...
	this.iconE.title = "Otevřít kalendář";
	this.visible = false;
}
...
function CalendarUpdate(date) {
	...
	if (this.template.yearsScroll) {
		navHtml += "<div id='" + this.id + "_nav_prevYear'" + this.RelatedDate(date, -1, "y") + " class='scrollButton left centerAlign middleVAlign pointer' title='Předchozí rok'>" +
				"<img src='" + this.config.imgPath + "prev_year.png' width='9' height='8' title='Předchozí rok' alt='' />" +
				"</div>";
	}
	
	navHtml += "<div id='" + this.id + "_nav_prevMonth'" + this.RelatedDate(date, -1) + " class='scrollButton left centerAlign middleVAlign pointer' title='Předchozí měsíc'>" +
			"<img src='" + this.config.imgPath + "prev_month.png' width='4' height='8' title='Předchozí měsíc' alt='' />" +
			"</div>" +
			"<div id='" + this.id + "_nav_title' class='left centerAlign middleVAlign'>" +
			this.template.months[date.getMonth()] + " " + date.getFullYear() +
			"</div>" +
			"<div id='" + this.id + "_nav_nextMonth'" + this.RelatedDate(date, 1) + " class='scrollButton left centerAlign middleVAlign pointer' title='Následující měsíc'>" +
			"<img src='" + this.config.imgPath + "next_month.png' width='4' height='8' title='Následující měsíc' alt='' />" +
			"</div>";
	
	if (this.template.yearsScroll) {
		navHtml += "<div id='" + this.id + "_nav_nextYear'" + this.RelatedDate(date, 1, "y") + " class='scrollButton left centerAlign middleVAlign pointer' title='Následující rok'>" +
				"<img src='" + this.config.imgPath + "next_year.png' width='9' height='8' title='Následující rok' alt='' />" +
				"</div>";
	}
